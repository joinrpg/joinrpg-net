namespace JoinRpg.Common.PrimitiveTypes;

public static class IdentificationParseHelper
{
    public static (int i1, int i2, int i3)? TryParse3(ReadOnlySpan<char> value, IFormatProvider? provider, params ReadOnlySpan<string> prefixes)
    {
        ReadOnlySpan<char> val = RemovePrefixes(value, prefixes);
        Span<Range> ranges = stackalloc Range[3];
        var count = SplitIdentifier(val, ranges);
        if (count == 3
           && int.TryParse(val[ranges[0]], provider, out var i1)
           && int.TryParse(val[ranges[1]], provider, out var i2)
           && int.TryParse(val[ranges[2]], provider, out var i3)
           )
        {
            return (i1, i2, i3);
        }

        return null;
    }

    public static (int i1, int i2)? TryParse2(ReadOnlySpan<char> value, IFormatProvider? provider, params ReadOnlySpan<string> prefixes)
    {
        ReadOnlySpan<char> val = RemovePrefixes(value, prefixes);
        Span<Range> ranges = stackalloc Range[2];
        var count = SplitIdentifier(val, ranges);
        if (count == 2
           && int.TryParse(val[ranges[0]], provider, out var i1)
           && int.TryParse(val[ranges[1]], provider, out var i2)
           )
        {
            return (i1, i2);
        }

        return null;
    }

    public static int? TryParse1(ReadOnlySpan<char> value, IFormatProvider? provider, params ReadOnlySpan<string> prefixes)
    {
        ReadOnlySpan<char> val = RemovePrefixes(value, prefixes);

        if (int.TryParse(val, provider, out var id) && id > 0)
        {
            return id;
        }

        return null;
    }

    public static ReadOnlySpan<char> RemovePrefixes(ReadOnlySpan<char> value, ReadOnlySpan<string> prefixes)
    {
        var val = value.Trim();
        foreach (var prefix in prefixes)
        {
            if (val.StartsWith(prefix))
            {
                var rest = val.Slice(prefix.Length);
                // Only strip prefix if followed by '(' or end of string (word boundary)
                if (rest.IsEmpty || rest[0] == '(')
                {
                    val = rest;
                    break;
                }
            }
        }
        if (!val.IsEmpty && val[0] == '(')
        {
            val = val.Slice(1);
        }
        if (!val.IsEmpty && val[val.Length - 1] == ')')
        {
            val = val.Slice(0, val.Length - 1);
        }

        return val;
    }

    public static (int i1, int i2, int i3, int i4)? TryParse4(ReadOnlySpan<char> value, IFormatProvider? provider, params ReadOnlySpan<string> prefixes)
    {
        ReadOnlySpan<char> val = RemovePrefixes(value, prefixes);
        Span<Range> ranges = stackalloc Range[4];
        var count = SplitIdentifier(val, ranges);
        if (count == 4
           && int.TryParse(val[ranges[0]], provider, out var i1)
           && int.TryParse(val[ranges[1]], provider, out var i2)
           && int.TryParse(val[ranges[2]], provider, out var i3)
           && int.TryParse(val[ranges[3]], provider, out var i4)
           )
        {
            return (i1, i2, i3, i4);
        }

        return null;
    }

    public static int SplitIdentifier(ReadOnlySpan<char> val, Span<Range> ranges) => val.SplitAny(ranges, "-,", StringSplitOptions.TrimEntries);
}
