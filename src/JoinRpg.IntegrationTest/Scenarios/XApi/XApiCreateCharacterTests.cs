...
public class XApiCreateCharacterTests(XApiMasterFixture fixture)
{
    ...
    [Fact]
    public async Task CreateNonPlayerHot_ThrowsException()
    {
        var projectId = await fixture.CreateNewProject(fixture.MasterUserId);
        await Should.ThrowAsync<HttpRequestException>(
            () => fixture.MasterClient.CreateCharacterAsync(projectId, new CreateCharacterRequest
            {
                CharacterType = CharacterTypeApi.NonPlayer,
                IsHot = true,
            }));
    ...
    public async Task WithoutAuth_Returns401()
    {
        var projectId = await fixture.CreateNewProject(fixture.MasterUserId);
        await Should.ThrowAsync<HttpRequestException>(
            () => fixture.AnonymousXApiClient.CreateCharacterAsync(projectId, new CreateCharacterRequest()));
    }
}
