    ...
    : CharacterOperationContext(ProjectInfo, Now, CurrentUser, FieldSaveHelper)
{
    ...
    /// <param name="responsibleMaster">Мастер, ответственный за новую заявку.</param>
    /// <param name="player">
    ...
    /// </param>
    public Claim NewClaim(ClaimStatus claimStatus, User responsibleMaster, User player)
        => NewClaimCore(claimStatus, responsibleMaster, player.UserId, player);

    private Claim NewClaimCore(
        ClaimStatus claimStatus,
        User responsibleMaster,
        int playerUserId,
        User? player)
    {
        var claim = new Claim
