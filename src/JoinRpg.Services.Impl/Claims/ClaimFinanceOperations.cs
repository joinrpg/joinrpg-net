...
internal static class ClaimFinanceOperations
{
    ...
    public static PendingComment AcceptFee(
        ...
        PaymentTypeInfo paymentType)
    {
        var pending = ctx.AcceptFeeDeferringNotification(contents, operationDate, money, paymentType);
        return ctx.EnqueueComment(pending);
    }

    /// <summary>
    /// То же, но уведомление в очередь не ставится — это делает вызывающий.
    /// </summary>
    /// <remarks>
    /// Нужно ровно там, где порядок отправки уведомлений не совпадает с порядком создания
    /// комментариев: при регистрации на игре деньги принимаются <b>до</b> смены статуса (приём гасит
    /// взнос), а уведомление о регистрации обязано уйти первым. Отдельное имя, а не
    /// <c>bool</c>-параметр: порядок отправки — часть контракта, и менять его надо явно.
    /// </remarks>
    public static PendingComment AcceptFeeDeferringNotification(
        this ClaimMutationContext ctx,
        string contents,
        DateTime operationDate,
        int money,
        PaymentTypeInfo paymentType)
    {
        ...
        var pending = ctx.CreateComment(ctx.Claim, contents, commentAction, claimOperationType)
