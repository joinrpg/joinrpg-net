...
public class ClaimServiceImplTest : ClaimServiceTestBase
{
    ...
    /// <summary>
    /// С деньгами уведомлений два, и порядок следует порядку операций: сначала приняли взнос, потом
    /// зарегистрировали.
    /// </summary>
    /// <remarks>
    /// До миграции рассылка шла наоборот — статусное письмо первым, финансовое вторым, — хотя
    /// операции выполнялись в обратном порядке. Порядок выровнен по операциям сознательно.
    /// </remarks>
    [Fact]
    public async Task CheckInClaim_WithMoney_SendsFinanceNotificationBeforeStatusOne()
    {
    ...
    /// <summary>
    /// Разрешение на чувствительные данные переносится со старой заявки: его даёт игрок, а мастер за
    /// него не может, так что иначе проект потребовал бы паспорт заново у того же игрока.
    /// </summary>
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public async Task MoveToSecondRole_CarriesSensitiveDataPermissionOver(bool allowed)
    {
        var (oldClaim, target) = CreateSecondRoleSetup();
        oldClaim.PlayerAllowedSenstiveData = allowed;

        _ = await CreateService().MoveToSecondRole(oldClaim.GetId(), target.GetId(), "вторая роль");

        NewClaimBesides(oldClaim).PlayerAllowedSenstiveData.ShouldBe(allowed);
    }
    ...
    /// <summary>
    /// Уведомлений два: по старой заявке (выход из игры) и по новой (вторая роль).
    /// </summary>
    /// <remarks>
    /// До миграции комментарий к новой заявке создавался молча, и этот тест фиксировал ответ
    /// «не слать». По ревью #4840 молчание снято: заглушать его не за что, комментарии второй роли
    /// ведут себя как все прочие. Порядок следует порядку создания — сначала старая заявка.
    /// </remarks>
    [Fact]
    public async Task MoveToSecondRole_NotifiesAboutBothClaims()
    {
        var (oldClaim, target) = CreateSecondRoleSetup();

        var newClaimId = await CreateService().MoveToSecondRole(oldClaim.GetId(), target.GetId(), "вторая роль");

        SentNotifications.Count.ShouldBe(2);

        var outOfGame = SentNotifications[0].ShouldBeOfType<ClaimSimpleChangedNotification>();
        outOfGame.ClaimId.ShouldBe(oldClaim.GetId());
        outOfGame.CommentExtraAction.ShouldBe(CommentExtraAction.OutOfGame);
        outOfGame.AnotherCharacterId.ShouldBe(target.GetId());

        var secondRole = SentNotifications[1].ShouldBeOfType<ClaimSimpleChangedNotification>();
