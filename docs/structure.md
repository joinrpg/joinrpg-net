...
Этот документ описывает, куда добавлять новый код и как устроены проекты решения.

## Соглашения по `.csproj`

Общие настройки лежат в `Directory.Build.props` в корне репозитория и применяются ко всем проектам
автоматически. Не дублировать их в отдельных `.csproj`. В частности:

- **`InternalsVisibleTo` не добавлять вручную.** `Directory.Build.props` уже открывает `internal`-типы
  каждого проекта `XXX` его тестовым проектам `XXX.Test` и `XXX.Tests`. Достаточно назвать тестовый
  проект по этому шаблону.
- `Nullable`, `ImplicitUsings`, `TargetFramework`, `GenerateDocumentationFile`, `WarningsAsErrors` и
  прочие общие свойства тоже заданы централизованно — не переопределять без необходимости.
