...
Для каждого нового вида уведомления (нового текста/шаблона, который видит пользователь) добавлять verify-тест
(Verify.Xunit, `.verified.txt` снапшот) на итоговый текст — см. `VerifyNotificationTest.cs`,
`AdminNotificationServiceImplTest.cs`, `VerifyPostboxBody.cs`, `VerifyTelegramBody.cs` как образцы. Снапшот ловит
баги форматирования/интерполяции, которые не видны при обычной юнит-проверке (например, случайный `ToString()`
typed-обёртки вместо `.Value`).
