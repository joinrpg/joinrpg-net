...
JoinRpgRoles.prototype.loadRoles = function(projectId, locationId) {
	$.ajax({
		url: '/' + projectId + '/roles/' + locationId + '/indexjson',
		dataType: 'json',
		type: 'GET',
		...
		error: function() {
			block.text('Не удалось загрузить ролевку');
		}
	})
};
...
JoinRpgRoles.prototype.appendGroup = function(block, group) {
	if (group.Characters.length > 0 || group.CanAddDirectClaim) {
		...
		if (group.CanAddDirectClaim) {
			var text = this.getVacanciesText(group.DirectClaimsCount);
	...
	for (var i=0; i<characters.length; i++) {
		...
		char.find('.joinrpg-role').html('<a href="'+character.CharacterLink+'">'+character.CharacterName+'</a> ')
			...
			.html(character.PlayerLink != null ? 
				...
				: ' ' +
				(character.ActiveClaimsCount > 0 ? '<span class="already-claimed">Подано заявок: '+character.ActiveClaimsCount+'</span><br>'  : '') + ' <a href="'+character.ClaimLink+'" class="joinrpg-claim-btn">Заявиться</a>');
		block.append(char);
	}
};
